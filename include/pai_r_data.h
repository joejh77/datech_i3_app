...
typedef enum{
	eMmbEventType_NONE = 0,
	eMmbEventType_SOS,
	eMmbEventType_RAPID_ACCELERATION,
	eMmbEventType_SUDDEN_DECELERATION,
	eMmbEventType_SUDDEN_HANDLE,
	eMmbEventType_GSENSOR,
	eMmbEventType_OVERSPEED,
	eMmbEventType_FASTSPEED,

	eMmbEventType_ACC_ON = 9,
	eMmbEventType_ACC_OFF,
...
typedef struct {
	...
	u8 		eventType; // 1:SOS, 2:급가속, 3:급감속, 4:급핸들, 5:충격, 6 over speed, 7 fast speed, 9:ACC ON, 10: ACC OFF
	u8 		gnssStatus; // 'A', 'V', 'P'
...
typedef struct {
	...
	char 	file_path[60]; //" /mnt/extsd/NORMAR/20001130_152107_I2.avi"
	time_t 	create_time;
	size_t 	move_filesize; //Byte
	THUMBNAIL thumbnail;
	...
	u8 		gnssStatus; // 'A', 'V', 'P'
	u8 		eventType; // 1:SOS, 2:급가속, 3:급감속, 4:급핸들, 5:충격, 9:ACC ON, 10: ACC OFF
#else
	u16		accurate;
#endif
	u16		direction; //0~359
...
}LOCATION_INFO; //for pai-r server
...
public:
	...
	u32 Location_queue_first_autid(eUserDataType ud_type = eUserDataType_Normal);
...
public:
	...
	int BackupLocationGet(PAI_R_BACKUP_DATA * ptrPai_r_data, eUserDataType ud_type = eUserDataType_Normal);
	int BackupLocationGetEvent(PAI_R_BACKUP_DATA * ptrPai_r_data, eUserDataType ud_type, time_t event_time);
protected:
	FILE *	m_read_fp;
	USER_DATA_HEADER hd[eUserDataType_End];
