...
//
			...
			pEleItem->SetAttribute( "value", spConfig->strCloudServerName);
		}
		else if(!strcmp(lpszId, "CloudServerPort")){
			pEleItem->SetAttribute( "value", spConfig->iCloudServerPort);
		}
		else if(!strcmp(lpszId, "CloudServerPollingPort")){
			pEleItem->SetAttribute( "value", spConfig->iCloudServerPollingPort);
		}
...
//
			...
			pEleItem->QueryIntAttribute( "value", &spConfig->iCloudServerPollingPort);
		}
...
#if BUILD_MODEL == BUILD_MODEL_VRHD_MMB_SERVER
	strcpy(pCfg->strSeriarNo, "2");
#else
...
#if BUILD_MODEL == BUILD_MODEL_VRHD_MMB_SERVER
	strcpy(pCfg->strCloudServerName, "skyeyedms.com");
	strcpy(pCfg->strCloudServerApiUrl, "");
	pCfg->iCloudServerPort = 15100;
	pCfg->iCloudServerPollingPort = 17100;
#else
	strcpy(pCfg->strCloudServerName, "");
	strcpy(pCfg->strCloudServerApiUrl, "");
	pCfg->iCloudServerPort = 0;
	pCfg->iCloudServerPollingPort = 0;
#endif
...
#if BUILD_MODEL == BUILD_MODEL_VRHD_MMB_SERVER
		"<item value=\"\" id=\"CloudServerName\"/>" //MMB Server
		"<item value=\"\" id=\"CloudServerPort\"/>"
		"<item value=\"\" id=\"CloudServerPollingPort\"/>"
#endif
		"</setup>";
