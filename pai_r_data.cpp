...
int CUserData::BackupLocation(eUserDataType ud_type)
{
	...
	if(data_count)
	{
		...
		data.location.create_time = iData->location.create_time;
		...
		data.location.gnssStatus = iData->location.gnssStatus;
		data.location.eventType = iData->location.eventType;
#else
...
u32 CUserData::Location_queue_first_autid(eUserDataType ud_type)
{
...
u32 CPai_r_data::Location_queue_first_autid(eUserDataType ud_type)
{
	u32 count = Location_queue_count(ud_type);
...
int CPai_r_data::Location_pop(PAI_R_BACKUP_DATA *pLoc, u32 auto_id,eUserDataType ud_type, bool continue_read)
{
	...
	if(ret == 0)
 	...
 	dbg_printf(DBG_PAIR_INFO, "%s read ( autoid %d : %d, %d ) out_id = %d \n", pLoc->location.file_path, pLoc->location.autoid, pLoc->event_autoid, auto_id, out_id);
...
{	
	...
	{
		int i = 0;
		u32 start_queue_no = Location_queue_first_autid(ud_type);
...
#ifdef DEF_MMB_SERVER
			loc_info.loc.append(format_string("\"gnssStatus\":\"%c\",", (char)ptrPai_r_data->location.gnssStatus));
			loc_info.loc.append(format_string("\"eventType\":%u,", (u32)ptrPai_r_data->location.eventType));
#else
...
int CPai_r_data::BackupLocationGet(PAI_R_BACKUP_DATA * ptrPai_r_data, eUserDataType ud_type)
{	
	int data_count = Location_queue_count(ud_type);
	...
	if(data_count)
	{
		...
		{			
			...
			if(ud_type)
				autoid = ptrPai_r_data->event_autoid;
			else
				autoid = ptrPai_r_data->location.autoid;

			if(cur_queue_no != autoid){
				dprintf(DBG_PAIR_ERR, "%s() : error!! autoid is different. %d (%d:%d)\r\n", __func__,  ptrPai_r_data->location.autoid, autoid, cur_queue_no);
			...
			loc.append("{");
			//loc_info.loc.append(format_string("\"driverecorder_autoid\":%u,", ptrPai_r_data->location.autoid/*hd[ud_type].in.c_str()*/));
			loc.append(format_string("\"driverecorder_autoid\":%u,", ptrPai_r_data->location.unique_local_autoid));
        	loc.append(format_string("\"create_datetime\":\"%s.%03u\",", make_time_string(ptrPai_r_data->location.create_time).c_str(),  (u32)ptrPai_r_data->location.create_time_ms));
        	loc.append(format_string("\"movie_filesize\":%u,", ptrPai_r_data->location.move_filesize));
        	loc.append(format_string("\"g_sensortype_id\":%d,", is_event));
			loc.append(format_string("\"latitude\":%0.6f,", ptrPai_r_data->location.latitude));
			loc.append(format_string("\"longitude\":%0.6f,", ptrPai_r_data->location.longitude));
#ifdef DEF_MMB_SERVER
			loc.append(format_string("\"gnssStatus\":\"%c\",", (char)ptrPai_r_data->location.gnssStatus));
			loc.append(format_string("\"eventType\":%u,", (u32)ptrPai_r_data->location.eventType));
#else
			loc.append(format_string("\"accurate\":%u,", ptrPai_r_data->location.accurate));
#endif	
			loc.append(format_string("\"direction\":%u,", ptrPai_r_data->location.direction));
			loc.append(format_string("\"altitude\":%u", ptrPai_r_data->location.altitude));
			loc.append("}");

			m_tmp_pop_count++;
#ifdef DEF_MMB_SERVER
			if(m_tmp_pop_count >= DEF_MMB_MAX_EVENTDATA_COUNT || (loc.length()) > (HTTP_MULTIPART_BUFFER_SIZE - (3 * 1024)))
#else
			if(loc.length() > (HTTP_MULTIPART_BUFFER_SIZE - (3 * 1024)))
#endif
				break;
		...
		dbg_printf(DBG_PAIR_INFO , "%s(file autoid : %d, queue_no : %d) \r\n", file_name.c_str(), ptrPai_r_data->location.unique_local_autoid, start_queue_no);
		dbg_printf(DBG_PAIR_INFO , "%s \r\n", loc.c_str());
...
int CPai_r_data::BackupLocationGetEvent(PAI_R_BACKUP_DATA * ptrPai_r_data, eUserDataType ud_type, time_t event_time)
{	
	int result = 0;
	int retry = 3;
	...
	if(event_time)
	{
		...
		loc = std::string("[");
		dprintf(DBG_PAIR_INFO, "t create_datetime : %s (s_q:%d)\r\n", make_time_string(event_time).c_str(), start_queue_no);
		
		for(i=0 ; i <= start_queue_no; i++)
		{			
			u32 autoid;
			cur_queue_no = start_queue_no - i;
						
			if(!Location_pop(ptrPai_r_data, cur_queue_no, ud_type)){
				dbg_printf(DBG_PAIR_ERR , "%s() : Location_pop error! (%d)\r\n", __func__, cur_queue_no);
				if(retry--)
					continue;
			...
			dprintf(DBG_PAIR_INFO, "c create_datetime : %s.%03u (%d : %d : %d)\r\n", \
				make_time_string(ptrPai_r_data->location.create_time).c_str(),  \
				(u32)ptrPai_r_data->location.create_time_ms, i, retry, cur_queue_no);
			...
			if(ptrPai_r_data->location.create_time > event_time){
				continue;
			}
			else if(ptrPai_r_data->location.create_time < event_time) {
				if(retry--)
					continue;
				break;
			}
		...
		dbg_printf(DBG_PAIR_INFO , "%s(file autoid : %d, queue_no : %d) \r\n", file_name.c_str(), ptrPai_r_data->location.unique_local_autoid, cur_queue_no);
		dbg_printf(DBG_PAIR_INFO , " %d %s \r\n", result, loc.c_str());
