...
//	bool gps_led_on = false;
	...
	class CBbApi bbApi(&fd_gps);
	bbApi.SerialOpen(GPS_DEVICE, GPS_BAUDRATE);
#endif
	...
	if(fd_key == NULL)
		i= 3;
    	fd_key = __evdev_input_open("ht-gpio-keys", i); //new board is 3

    if ( !__evdev_new_from_fd_has_event_code(fd_key, &dev_key, EV_SW, SW_DOCK)) {
		//error
		dev_key = NULL;
		goto exit;
    }
...
#if USE_DA_PULSE
	i= 0;
	fd_da = __evdev_input_open("pulse_count", i); //new board is 0

    if ( !__evdev_new_from_fd_has_event_code(fd_da, &dev_da, EV_REL, REL_X)) {
		l_rdk_board = true;
		//error
		dev_da = NULL;
    }
...
#if USE_KEY	
	for( i = 0; i < 6; i++){
		int value = libevdev_get_event_value(dev_key, EV_SW, i);
		if(value == 0)
			printf("%d is low\n", i);
		else
			printf("%d is high\n", i);
    ...
    do {
        struct input_event ev;
		 ...
		 u32 l_pulse_tmout_ms = 0;
		 u32 l_rpm_tmout_ms = 0;
		 u32 l_speed_tmout_ms = 0;
		 char gps_data[256] = { 0, };
		 u32 gps_data_index = 0;
		 //u32 pulse_count_cheak = 0; //osaka gas test
		...
		while(evdev_thread_run){
			...
			len = read_serial(fd_gps, data, sizeof(data), 0);
	       ...
	       if (rc == 0) {
				...
				if (ev.type == EV_REL) {
					 switch (ev.code) {
						...
						case REL_Y:
							if(l_pulse_data.m_pulse_count_accrue == ev.value){
								l_pulse_data.m_fPulseSpeed = 0.0;
								l_pulse_data.m_iPulseSec = 0;
							}
							else
								l_pulse_data.m_pulse_count_accrue = ev.value;

							//printf("REL_Y : %d:%d\n", ev.code, ev.value);
#ifdef DEF_OSAKAGAS_USERDATA_1SEC
							if(is_time_out_ms(&l_pulse_tmout_ms, 900)){
								evdev_input_pulse_data_add_stream();
								printf("evdev_input_pulse_data_add_stream excute 1 \n");
							}
#else
							evdev_input_pulse_data_add_stream();
#endif
						...
						case REL_RX:
							l_pulse_data.m_iRpmPulseSec = (int)ev.value;
						...
						case REL_RY:
							...
							if(l_pulse_data.m_rpm_count_accrue == ev.value){
								l_pulse_data.m_iRpm = 0;
								l_pulse_data.m_iRpmPulseSec = 0;
							}
							else
								l_pulse_data.m_rpm_count_accrue = ev.value;
