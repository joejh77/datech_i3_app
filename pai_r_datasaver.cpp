...
int pai_r_datasaver_addLocation_new(std::string& file_path, u8 event_type, int pre_recording_time)
{
		eUserDataType dataType = event_type ? eUserDataType_Event : eUserDataType_Normal;
		PAI_R_LOCATION loc;
		...
		if(loc.create_time == 0)
			time(&loc.create_time);
		loc.create_time_ms = (get_tick_count()%1000);
		loc.move_filesize = 0; //KBYTE

		loc.eventType = event_type;
		loc.gnssStatus = gps.m_bValid ? 'A' : 'V';
	...
	return pai_r_data.addLocation(loc, dataType, pre_recording_time);
}
