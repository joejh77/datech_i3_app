...
#define offs_write_alignemnt_size "32768"
...
#define DISPLAY_PIP_CONTROL_USE		0 // 가끔 Rear 화면이 보이지 않는 문제가 있어 사용하지 않음
#define DISPLAY_PIP_SIZE 	3 // 2 is 1/4 size
#define DISPLAY_PIP_FRONT_CAMERA_ONLY 	60

#define REAR_CAMERA_CHECK_ONFRAME		1
#define FRONT_CAMERA_1080P_FIX				1 // 720p는 스케일링하여 저장
...
static u32 l_doing_file_save_tm = 0;
#endif
...
void Recorder_datalog_add(time_t t)
{
	...
	if(l_datalog.m_last_add_time != t)
		safe_driving_monitoring_osakagas(); // 호출 위치 변경
#endif

#ifdef ENABLE_DATA_LOG_UNI
		if(l_sys_setup.isInit()){//test // 농공대학 
			...
			//printf("start1 rtc = %s\r\n", make_recording_time_string(start.tv_sec).c_str());
			//gettimeofday(&start, NULL);
			//printf("start2 rtc = %s\r\n", make_recording_time_string(start.tv_sec).c_str());
			
			l_datalog.m_last_add_time = t;
			
			log_data.Time = start.tv_sec;		 /* log time */
			log_data.milisec_time = start.tv_usec / 1000;
		...
		if(l_datalog.m_last_add_time != t && l_sys_setup.isInit()){
			...
			//if(log_data.Cause == EVENT_CAUSE_CATEGORY_NO_EVENT){
			//	log_data.Cause = l_datalog.m_event_cause_category; // 위치변경
			//	log_data.Difference = l_datalog.m_event_cause_difference;
			//}
			log_data.Highway = (int)safe_driving_get_highway_mode();
#else
...
#ifdef DEF_PAI_R_DATA_SAVE
	l_doing_file_save_tm = get_tick_count();
#endif		
#if (defined(DEF_VRSE_SECURITY_FILE_EXT) && ENABLE_AVI_SCRAMBLING == 0)
		if(strlen(l_normal_recording_file_path)) {
			u32 idx = l_st_motion_recording_file_path.idx % FILE_PATH_BUFFER_COUNT;
...
#define ENABLE_REAR_NO_SIGNAL 	0
	...
	virtual void queryNewFilePath(void* user_data, oasis::recording_mode_t file_type, bool rear_camera_on, bool sound_on, std::string& file_path) {
		...
		int result = sscanf(file_path.c_str(), VIDEO_NORMAL_DIR "/%4d_%02d_%02d_%02d_%02d_%02d", &tm_t.tm_year, &tm_t.tm_mon, &tm_t.tm_mday, &tm_t.tm_hour, &tm_t.tm_min, &tm_t.tm_sec);
		
		if(result == 6){
			tm_t.tm_year -= 1900;
			tm_t.tm_mon -= 1;
			file_path = Recorder_get_recording_file_name(VIDEO_NORMAL_DIR, tm_t, l_normal_recording_type, rear_disabled_ ? 1 : 2, ext);
		}
		else {
	 		//std::string new_file_path = std::strtok((char *)file_path.c_str(), ".");
			file_path.resize(file_path.size() - 4);
				
			file_path = oasis::format("%s_%c%d.%s", file_path.c_str(), l_normal_recording_type, rear_disabled_ ? 1 : 2, ext.c_str());
		}
...
#ifdef DEF_PAI_R_DATA_SAVE
		if(l_net_exist){
			pai_r_datasaver_addLocation_new(file_path, 0, 0);
...
class MyPreviewObserver : public ui::PreviewObserver {
  public:
	MyPreviewObserver() { }
	...
	//adas
	virtual void onAdasCarCrashWarning(int32_t camera_id, CarInfo& car) {
		DLOG(DLOG_ADAS, "car !!!!!!!!!!!!!!!!!!!!\n");
	}


	virtual void onAdasLane(int32_t camera_id, LaneInfo& lane, unsigned char lt_warn, unsigned char rt_warn) {
		if(lane.isDisp) {
			DLOG(DLOG_ADAS, "lane ______________________\n");
		}
		if(rt_warn != 0) {
			DLOG(DLOG_ADAS|DLOG_INFO, "right[%d] >>>>>>>>>>>>>>>>>>>>>>>>\n", rt_warn);
		}
		if(lt_warn != 0) {
			DLOG(DLOG_ADAS|DLOG_INFO, "left[%d] <<<<<<<<<<<<<<<<<<<<<<<<<\n", lt_warn);
		}
	}

	virtual void onAdasCars(int32_t camera_id, int32_t num_cars, CarInfo* cars) {
		DLOG(DLOG_ADAS, "cars#%d ooooooooooooooooooooo\n", num_cars);
	}

	virtual void onAdasPlates(int32_t camera_id, int32_t num_plates, PlateInfo* plates) {
		DLOG(DLOG_ADAS, "plates#%d [-----------------------]\n", num_plates);
	}

	virtual void onAdasSaveData(int32_t camera_id, uint16_t vanishline, uint8_t vanishline_ok, int32_t width, int32_t height, uint16_t *data) {
		DLOG(DLOG_ADAS|DLOG_INFO, "()\n");
	}
	
	virtual void ponRearAdasCarCrashWarning(int32_t camera_id) {
		DLOG(DLOG_ADAS|DLOG_WARN, "rear car crash warning !!!!!!!!!!!!!!!!!!!!\n");
	}
...
class MyFrameObserver : public ui::PreviewFrameObserver {
  public:
	...
	void onFrame(int32_t camera_id, uint8_t *frame, int32_t stride_width, int32_t width, int32_t height)
	{
...
#ifdef ENABLE_DATA_LOG_UNI
		//frame_cnt++;
		//if(camera_id == 0 && ((frame_cnt % 2) == 0)){ //test
		//	printf("frame_cnt = %d\r\n", frame_cnt);
		...
		if(camera_id == l_recorder_camera2_id && ((frame_cnt++ % 3) == 0)){
			//fram yuv420
			//n = Height x Width
			//Y=n | Cb=n/4 | cr=n/4
			...
			if(value == (16 * size)){ // 16 is no signal pixel
				...
				if(rear_camera_detected){
					if(Video2error++ >= 50){ // 5sec
						rear_camera_detected = false;
						
						DLOG0(DLOG_ERROR, " error!, rear camera disconnected. (%d)\r\n", value );
					}
				}
			}
			else{
				Video2error= 0;
				if(rear_camera_detected == false){
					if(value > (16 * size)) //Aging시 오동작이 발생하여 밝기가 확실한 경우만 2Ch로 전환
						Video2OkCnt++;
					else
						Video2OkCnt = 0;
					...
					if(Video2OkCnt >= 10){
						...
						DLOG0(DLOG_WARN, " rear camera detected. (%d)\r\n", value );
					}
				}
			}
...
//#endif
		...
		if(l_sys_setup.cfg.iVideoQuality == CFG_VIDEO_QUALITY_HIGH){
			...
			pEnv->front_fps = 10; // CAMERA_FPS 정수 배로 지정 : 1,2,3,5,6,10,15
			pEnv->rear_fps = 5;
...
#if 0 //bitrate test 16M File 6FPS
		if(l_sys_setup.cfg.iVideoQuality == 0){
			//pEnv->front_bitrate += 0;
			pEnv->rear_bitrate -= BITRATE_720P;
		}
#endif
...
int recorder_low_battery_check(void)
{
	...
	int volt = (int)l_sub_mcu.m_volt;

#ifdef DEF_PAI_R_DATA_SAVE	
	if(l_doing_file_save_tm + 1000 < get_tick_count())
#endif		
	...
	if(l_sys_setup.cfg.bCarBatSafeEnable && l_is_parking_mode){
		...
		if(power_low_voltage == false && volt < safe_volt) {
			if( ++low_battery_count >= LOW_BATTERY_ENABLE_TIME){
				...
				std::string _name("/acc_off");
				pai_r_datasaver_addLocation_new(_name, eMmbEventType_ACC_OFF, 0);
#endif				
...
//		}
		...
		sleep(3);
		//timer_running_ = false;
		recorder_reboot();//OnOFF Test_230420
	}
...
#if 0
		//20220920 업데이트 후 RearCAM 720p 고정
		l_sub_mcu.SetEepromData(EEPROM_HW_CONFIG, CFG_RESOLUTION_720P); //Master update for ViewTec
	...
	do {
			...
			if (recorder_is_sd_exist()) {
		       ...
		       l_sd_exist = true;
				...
				if(is_time_out_ms(&l_sd_error_tm_ms, 5000)){
					...
					if(error_count++ < 3 || is_sd_error ) { //카드 미삽입(3회 반복_5s 주기), 녹화 중 에러 (무한반복_5s 주기)
		...
		if(l_sd_exist && is_recording_start == false)		
		{
			...
			if(!l_sys_setup.Init()){
				...
				system("fsck.offs -x /dev/mmcblk0p1");
				//system("fsck.offs -x /dev/mmcblk0p2");	//test joe
				//iResult = system("fsck.offs -x /dev/mmcblk0p1");//test
/* 
/tmp/offs.info
...
*/
				sysfs_scanf("/tmp/offs.info", "version=%f\nresult=%d", &dOffsVer, &iResult); //test
				DLOG(DLOG_INFO, "OFFS version=%f	OFFS result=%d\r\n",dOffsVer, iResult);
				if(iResult != 0) { 
					do {
						if(is_time_out_ms(&l_sd_error_tm_ms, 5000)){
							if(error_count == 0)
							...
							if(error_count++ < 5 || is_sd_error ) { //카드 미삽입(5회 반복_5s 주기), 녹화 중 에러 (무한반복_5s 주기)
								mixwav_play(kMixwaveSDError);
							}
							else {
								led_process_postCommand(LED_WORK_MODE, LED_WORK_LED_OFF, LED_TIME_INFINITY);
...
void camera_parameters_set(oasis::key_value_map_t &parameters, int32_t camera1_id, int32_t camera2_id, int32_t rotation, std::string &ext, REC_ENV &rec_env, std::string &res_front, std::string &res_rear, bool audio_enable)
{
	// config cameras
	parameters.clear();

	//front camera settings
	parameters["front-camera-id"] = std::to_string(camera1_id);
	parameters["front-capture-buffers"] = std::to_string(CAPTURE_BUFFER_NUM);

#ifdef DEF_REAR_CAMERA_ONLY
 #if REAR_NTSC
			//parameters["rear-capture-width"] = "720";
			parameters["front-capture-width"] = "1920"; //tp2825-sepcific
			parameters["front-capture-height"] = "480";
			parameters["front-capture-interlaced"] = "1";
 #else
			if((rec_env.hw_config & HW_CONFIG_REAR_CAM_TYPE_MASK) == CFG_RESOLUTION_720P){
				parameters["front-capture-resolution"] = "720p";
			}
			else if((rec_env.hw_config & HW_CONFIG_REAR_CAM_TYPE_MASK) == CFG_RESOLUTION_1080P){
				parameters["front-capture-resolution"] = "1080p";
			}
			else{
				parameters["front-capture-width"] = "1920"; //tp2825-sepcific
				parameters["front-capture-height"] = "480";
 ...
 #endif
#else
 #if FRONT_CAMERA_1080P_FIX
	if(camera1_id == 0 && camera2_id == 1){
		parameters["front-capture-resolution"] = "1080p";
	}
	else
		parameters["front-capture-resolution"] = res_front.c_str();
 #else
	parameters["front-capture-resolution"] = res_front.c_str();
 #endif
#endif 
	
	parameters["front-fps"] = std::to_string(CAMERA_FPS);
	parameters["front-subchannel-rotation"] = std::to_string(rotation);

	if(rear_disabled_ == false) 	
	{
		//rear camera settings
		parameters["rear-camera-id"] = std::to_string(camera2_id);
		parameters["rear-capture-buffers"] = std::to_string(CAPTURE_BUFFER_NUM);

#if REAR_NTSC
			//parameters["rear-capture-width"] = "720";
			parameters["rear-capture-width"] = "1920"; //tp2825-sepcific
			parameters["rear-capture-height"] = "480";
			parameters["rear-capture-interlaced"] = "1";
#else
			if((rec_env.hw_config & HW_CONFIG_REAR_CAM_TYPE_MASK) == CFG_RESOLUTION_720P){
				parameters["rear-capture-resolution"] = "720p";
			}
			else if((rec_env.hw_config & HW_CONFIG_REAR_CAM_TYPE_MASK) == CFG_RESOLUTION_1080P){
				parameters["rear-capture-resolution"] = "1080p";
			}
			else{
				parameters["rear-capture-width"] = "1920"; //tp2825-sepcific
				parameters["rear-capture-height"] = "480";
		...
		parameters["rear-fps"] = std::to_string(CAMERA_FPS);
		//parameters["rear-subchannel-rotation"] = "0";
	}
}

void preview_parameters_set(oasis::key_value_map_t &parameters, int32_t camera1_id, int32_t camera2_id, int32_t rotation, std::string &ext, REC_ENV &rec_env, std::string &res_front, std::string &res_rear, bool audio_enable)
{
//	int32_t fps;
	...
	//*******************************************************************************
	parameters["motion-sensitivity-level"] = oasis::format("%d", rec_env.motionSensi); // 1~6
		
#if ENABLE_MD
 #if ENABLE_BOARD_CHECK
 	parameters["enable-motion-detect"] = "1";
	parameters["motion-detect-fps"] = "6";
	parameters["motion-detect-version"] = "2";
	parameters["motion-detect-height-max"] = "100";
 #else
	if(rec_env.bMotionRecEnable){
		...
		parameters["enable-motion-detect"] = "0";
 #endif	
#else
	parameters["enable-motion-detect"] = "0";
#endif
#if ENABLE_ADAS
	parameters["enable-adas"] = "1";
	parameters["adas-gps-enabled"] = "0";
	parameters["adas-gsensor-enabled"] = "1";
	parameters["adas-force-default-view-angle"] = "0";
	parameters["adas-default-horz-view-angle"] = "54";
	parameters["adas-default-vert-view-angle"] = "39";
	parameters["adas-collision-avoidance-sensitivity"] = "0";
	parameters["adas-lane-departure-sensitivity"] = "0";
	//parameters["adas-fps"] = "30";
	adas_enabled_ = true;
#else
	parameters["enable-adas"] = "0";
	parameters["enable-rear-adas"] = "0";
#endif
...
void recorder_parameters_set(oasis::key_value_map_t &parameters, int32_t camera1_id, int32_t camera2_id, int32_t rotation, std::string &ext, REC_ENV &rec_env, std::string &res_front, std::string &res_rear, bool audio_enable)
{
	...
	parameters["file-prefix"] = oasis::format("");

	//*****************************************************************************************
	//*****************************************************************************************
	//*****************************************************************************************
	parameters["file-extension"] = ext;

	parameters["driving-folder-path"] = VIDEO_NORMAL_DIR;
	parameters["event-folder-path"] = VIDEO_EVENT_DIR;

#if ENABLE_MD	
	parameters["motion-folder-path"] = VIDEO_MOTION_DIR;
	//parameters["motion-folder-path"] = "/mnt/extsd/PARKING";
#endif

	//++ 2020/06/14
	parameters["save-stream-data-in-header"] = "1"; // 1은 header에 바로 저장 됨
	...
	parameters["recording-size-limit-threshold-seconds"] = std::to_string(RECORDING_SIZE_LIMIT_SECONDS);
#if 1
	parameters["event-pre-recording-seconds"] = std::to_string(PRE_RECORDING_SECONDS);
	parameters["event-post-recording-seconds"] = std::to_string(POST_RECORDING_SECONDS);

	parameters["motion-pre-recording-seconds"] = std::to_string(PRE_RECORDING_SECONDS);
	parameters["motion-post-recording-seconds"] = std::to_string(POST_RECORDING_SECONDS_MOTION);
#else
	parameters["event-pre-recording-seconds"] = "10";
	parameters["event-post-recording-seconds"] = "5";

	parameters["motion-pre-recording-seconds"] = "10";
	parameters["motion-post-recording-seconds"] = "5";
#endif
	...
	if(rotation == 0 && G_sensorGetControl()->rotate == DX_ROTATE_180){
		parameters["front-hflip"] = "1";
		parameters["front-vflip"] = "1";
	}
#endif

	parameters["front-h264-profile"] = "high";
	parameters["front-h264-level"] = "level51";

	if(rec_env.front_resolution == CFG_RESOLUTION_480I){
		parameters["front-out-width"] = "640";
		parameters["front-out-height"] = VGA_HEIGH_RESOLUTION;
	}
	else
		parameters["front-resolution"] = res_front.c_str();
#if 1
	parameters["front-bitrate"] = std::to_string(rec_env.front_bitrate);
#else
	if(camera2_id >= 0) // 2ch recording
		parameters["front-bitrate"] = std::to_string(rec_env.front_bitrate);
	else {// 1 ch recording
		if((rec_env.front_bitrate + rec_env.rear_bitrate < MBYTES(9)) && rec_env.front_fps == CAMERA_FPS)
			parameters["front-bitrate"] = std::to_string(rec_env.front_bitrate + rec_env.rear_bitrate);
		else
			parameters["front-bitrate"] = std::to_string(rec_env.front_bitrate);
	}
#endif
...
#if 0
	parameters["front-fps"] = std::to_string(fps);
#else
	parameters["front-fps"] = std::to_string(CAMERA_FPS);
	
	if(fps != CAMERA_FPS){
		parameters["front-h264-framerate"] = std::to_string(fps);

		if(fps == TIMELAPSE_FPS)
			fps = AVI_FPS;	//timelapse mode
		
		parameters["front-avi-framerate"] = std::to_string(fps);
	}
#endif

	parameters["front-h264-keyframe-interval"] = std::to_string(fps); //"30";
	parameters["front-h264-enable-cabac"] = "1"; //to enable "1"0
#if DEF_VIDEO_FIX_BITRATE
		parameters["front-h264-enable-fixqp"] = "1";
	//parameters["front-h264-fix-iqp"] = "20";
	//parameters["front-h264-fix-pqp"] = "35";
	parameters["front-h264-fix-iqp"] = std::to_string(rec_env.fix_iqp);	//"20";
	parameters["front-h264-fix-pqp"] = std::to_string(rec_env.fix_pqp);	//"38";
#else
	parameters["front-h264-min-qp"] = "27";
	parameters["front-h264-max-qp"] = "35";	//oasis 4.2.8 31=>42
	parameters["front-h264-enable-fixqp"] = "0";
	parameters["front-h264-fix-iqp"] = "10";
	parameters["front-h264-fix-pqp"] = "20";
#endif
	


	parameters["front-media-wait-timeout-secs"] = "3";
	parameters["front-media-wait-timeout-notify-oneshot"] = "1";
	...
	if(camera2_id >= 0) {
		...
		parameters["rear-h264-profile"] = "baseline";
		parameters["rear-h264-level"] = "level51";
		...
		if(rec_env.rear_resolution == CFG_RESOLUTION_480I){
			parameters["rear-out-width"] = "640";
			parameters["rear-out-height"] = VGA_HEIGH_RESOLUTION;
			
			DLOG0(DLOG_WARN, "rear-out : 640x%s\r\n", VGA_HEIGH_RESOLUTION);
		}
//		else if(rec_env.rear_resolution == CFG_RESOLUTION_720P){
//			parameters["rear-out-width"] = "1280";
//			parameters["rear-out-height"] = "720";
//		}
		else {
			parameters["rear-resolution"] = res_rear.c_str();
			DLOG0(DLOG_WARN, "rear-resolution : %s\r\n", res_rear.c_str());
		}

		parameters["rear-bitrate"] = std::to_string(rec_env.rear_bitrate);

#if 0
		parameters["rear-fps"] = std::to_string(rec_env.rear_fps);
#else
		parameters["rear-fps"] = std::to_string(CAMERA_FPS);
		...
		if(fps != CAMERA_FPS){
			parameters["rear-h264-framerate"] = std::to_string(fps);

			if(fps == TIMELAPSE_FPS)
				fps = AVI_FPS;	//timelapse mode
				
			parameters["rear-avi-framerate"] = std::to_string(fps);
		}
#endif
		parameters["rear-h264-keyframe-interval"] = std::to_string(fps); //"30";
		parameters["rear-h264-enable-cabac"] = "0";
#if DEF_VIDEO_FIX_BITRATE		
		parameters["rear-h264-enable-fixqp"] = "1";
		//parameters["rear-h264-fix-iqp"] = "20";
		//parameters["rear-h264-fix-pqp"] = "35";  //20 File

		parameters["rear-h264-fix-iqp"] = std::to_string(rec_env.fix_iqp + 1); //"25";
		parameters["rear-h264-fix-pqp"] = std::to_string(rec_env.fix_pqp + 2); // "40";  //16M File
#else		
		parameters["rear-h264-min-qp"] = "27";
		parameters["rear-h264-max-qp"] = "35";	//oasis 4.2.8 31=>42
		parameters["rear-h264-enable-fixqp"] = "0";
		parameters["rear-h264-fix-iqp"] = "10";
		parameters["rear-h264-fix-pqp"] = "20";
#endif
		

		parameters["rear-media-wait-timeout-secs"] = "3";
		parameters["rear-media-wait-timeout-notify-oneshot"] = "1";
...
//	if(l_sys_setup.cfg.iVideoQuality != 0)
	{
		...
		int h264_qp =  33;
 #if DEF_30FIX_FRAME	
		//if(rec_env.front_bitrate > MBYTES(6))
		if(l_sys_setup.cfg.iVideoQuality == 2)
			h264_qp = 33;		//qp 26 주행 중 88M 파일 31초만 녹화 됨
		else if(l_sys_setup.cfg.iVideoQuality == 1)
			h264_qp = 35;	//37;
		else // LOW
  #if 1//REAR_NTSC
			h264_qp = 35;	//37;
  ...
  #endif
 #else
		if(l_sys_setup.cfg.iVideoQuality == 0) //LOW
			h264_qp = 32;
 #endif
		// 0~51
		parameters["front-h264-min-qp"] = std::to_string(h264_qp);
		parameters["front-h264-max-qp"] = std::to_string(h264_qp);
		parameters["rear-h264-min-qp"] = std::to_string(h264_qp);
		parameters["rear-h264-max-qp"] = std::to_string(h264_qp);
	
		parameters["enable-dynamic-qp-range"] = std::to_string(qp_range);
	}
//#else
//	else
//		parameters["enable-dynamic-qp-range"] = std::to_string(1);
		...
		parameters["rear-h264-enable-fixqp"] = "1";
		parameters["rear-h264-fix-iqp"] = std::to_string(rec_env.fix_iqp); //"25";
		parameters["rear-h264-fix-pqp"] = std::to_string(rec_env.fix_pqp); // "40";  //16M File
#else
	if(l_sys_setup.cfg.iVideoQuality == CFG_VIDEO_QUALITY_HIGH){
		if(rec_env.rear_resolution == CFG_RESOLUTION_480I)
			parameters["rear-h264-min-qp"] = "15";
		else
			parameters["rear-h264-min-qp"] = "25";
		parameters["enable-dynamic-qp-range"] = std::to_string(1);
	}
	if(l_sys_setup.cfg.iVideoQuality == CFG_VIDEO_QUALITY_MIDDLE){
#if 1
		parameters["front-h264-min-qp"] = "35";
		parameters["rear-h264-min-qp"] = "35";	
...
#else
		//parameters["front-h264-min-qp"] = "29";
	...
	#if 1
		parameters["front-h264-min-qp"] = "34";  //35는 화질이 별로라 34로 Fix
		parameters["rear-h264-min-qp"] = "32";
		parameters["enable-dynamic-qp-range"] = std::to_string(0);
	#else
		//parameters["front-h264-min-qp"] = "31"; //"27"; //"23";
		//parameters["rear-h264-min-qp"] = "10";
		
		if(rec_env.rear_resolution == CFG_RESOLUTION_480I)
 			parameters["rear-h264-min-qp"] = "25"; //20M "21";
 		else
			parameters["rear-h264-min-qp"] = "29";

		parameters["enable-dynamic-qp-range"] = std::to_string(1);
	#endif
	}
#if 0 //def DEF_REAR_CAMERA_ONLY
	if(rec_env.front_resolution == CFG_RESOLUTION_480I){
		if(l_sys_setup.cfg.iVideoQuality == CFG_VIDEO_QUALITY_MIDDLE )
			parameters["front-h264-min-qp"] = "23";
		else if(l_sys_setup.cfg.iVideoQuality == CFG_VIDEO_QUALITY_LOW )
			parameters["front-h264-min-qp"] = "23";
	}
	else
		parameters["front-h264-min-qp"] = parameters["rear-h264-min-qp"];
#endif

 #ifdef DEF_REAR_CAMERA_ONLY	//test
	parameters["front-h264-min-qp"] = "25"; //"27"; //"23";
	parameters["rear-h264-min-qp"] = "25";
	...
	//osd settings
 	...
 	if((l_rec_env.hw_config & HW_CONFIG_REAR_CAM_TYPE_MASK) == CFG_RESOLUTION_1080P)
		parameters["front-osd-font-size"] = "36"; //in point size
	else if((l_rec_env.hw_config & HW_CONFIG_REAR_CAM_TYPE_MASK) == CFG_RESOLUTION_720P)
		parameters["front-osd-font-size"] = "26"; //in point size
	else
		parameters["front-osd-font-size"] = "26"; //in point size
 #else
//	if(rec_env.front_resolution == CFG_RESOLUTION_480I)
//		parameters["front-osd-font-size"] = "36"; //in point size
//	else if(rec_env.front_resolution == CFG_RESOLUTION_720P)
//		parameters["front-osd-font-size"] = "27"; //in point size
//	else
		parameters["front-osd-font-size"] = "36";  //"18"; //in point size
 #endif
#else
	//* 720p는 1920 => 720 scaling 됨 (최신 버전에선 정상 동작 함 ?)
	if(strcasecmp(res_front.c_str(), "1080p")  == 0)
		parameters["front-osd-font-size"] = "18"; //in point size
	else
		parameters["front-osd-font-size"] = "12"; //in point size
#endif

#if 1
	if(strcasecmp(res_rear.c_str(), "1080p")  == 0)
		parameters["rear-osd-font-size"] = "18"; //in point size
	else
		parameters["rear-osd-font-size"] = "12"; //in point size
#else
	parameters["rear-osd-font-size"] = "0"; //disable
#endif

#if FIXED_FONT_USED
	parameters["osd-font-face"] = "Consolas";
#else
	parameters["osd-font-face"] = "Serif";
#endif
	parameters["osd-text-color"] = "255,255,255"; //r,g,b in decimal
	parameters["osd-outline-color"] = "0,0,0"; //a,r,g,b
	parameters["osd-horz-align"] = "left"; //left, center, right
	parameters["osd-vert-align"] = "bottom"; //top, center, bottom
#if FIXED_FONT_USED
	parameters["osd-font-path"] = DEF_FONT_DIR "fonts/consola.ttf";
#else	
	parameters["osd-font-path"] = DEF_FONT_DIR "fonts/GenBkBasB.ttf";
	//parameters["osd-font-path"] = "/mnt/extsd/GenBasR.ttf";
#endif
}
...
CAMERA_CHECK:	
		...
		if(SB_CmdScanf("cat /sys/devices/tp2825/cvstd", "%s", cvstd)){
			u32 res = CFG_RESOLUTION_480I;

			DLOG(DLOG_TRACE, "Rear Camera Resolution %s\r\n", cvstd);
			
			if(!strcmp("720p30", cvstd) || !strcmp("720p30v2", cvstd)){
				res = CFG_RESOLUTION_720P;
			}
			else if(!strcmp("1080p30", cvstd)){
...
int main(int argc, char* argv[]) {
	...
	if(argc  < 5){
		int c;
		CDaSystemSetup sys_setup;
		sys_setup.Init();
		do {		c = tty_.tty_getchar(); }while( c < 0);
		sys_setup.Deinit();
		return -1;
	}
#endif
	...
	if (argc < 5) {
		DLOG0(DLOG_INFO, "USAGE: %s <avi|mp4> <front camera1 id> <rear camera2 id> <rotation> [front resolution] [rear resolution] [recording duration in secs]\nExample: %s avi 0 1 270\nto disable rear, %s avi 0 -1 270\n", argv[0], argv[0], argv[0]);
		return -1;
	}
...
#if 0
	parameters["offs-normal-qsize"] = "10";
	parameters["offs-event-qsize"] = "10";
#else
	parameters["offs-qsize-max"] = "8192"; //8MB
#endif
	parameters["offs-write-chunk-size"] = std::to_string(OFFS_WRITE_CHUNK_BYTES);
	parameters["offs-cache-size"] = "1024";

	parameters["offs-overwrite-if-exist"] = "1";
	parameters["offs-write-alignemnt"] = offs_write_alignemnt_size; //"offs-write-alignemnt": 기본값은 16384 입니다. 512배수 값이어야 합니다. 0 이면 disable합니다.
	//parameters["offs-write-alignemnt"] = "0";
	
	parameters["media-cache-size"] = "8192";
	...
	if (strcasecmp(ext.c_str(), "avi") && strcasecmp(ext.c_str(), "mp4")) {
		//DLOG(DLOG_ERROR, "file extension must be either \"avi\" or \"mp4\"\r\n");
		//return -1;
	}
...
#ifdef DEF_REAR_CAMERA_ONLY
	l_recorder_camera1_id = 1;
	l_recorder_camera2_id = -1;
#else
	l_recorder_camera1_id = atoi(argv[2]);
	l_recorder_camera2_id = atoi(argv[3]);
#endif
	...
	if(argc > 5) {
		if(strcasecmp(argv[5], "720p") == 0 || strcasecmp(argv[5], "1080p") == 0){
			res_front = argv[5];
			DLOG(DLOG_INFO, "front-capture-resolution: %s\n", res_front.c_str());
		}
	}
	...
	if(argc > 6) {
		if(strcasecmp(argv[6], "720p") == 0 || strcasecmp(argv[6], "1080p") == 0){
			res_rear = argv[6];
			DLOG(DLOG_INFO, "rear-capture-resolution: %s\n", res_rear.c_str());
		}
	}
	
	if(argc > 7) {
		l_rec_env.rec_secs = atoi(argv[7]);
		DLOG(DLOG_INFO, "recording duration: %d secs\n", l_rec_env.rec_secs);
	}

	if(argc > 8) {
		l_rec_env.httpdDebugEnable = atoi(argv[8]);
		DLOG(DLOG_INFO, "Httpd Debug Enable: %d \n", l_rec_env.httpdDebugEnable);
	}
...
*/
...
#if 1
	camera_parameters_set(parameters, l_recorder_camera1_id, l_recorder_camera2_id, rotation, ext, l_rec_env, res_front, res_rear, l_rec_env.bAudioRecEnable);
	configCameras(parameters);
	//sleep(1);
#endif	
	////////////////////////////////////////////////////////////////////////////////////////////
	preview_parameters_set(parameters, l_recorder_camera1_id, l_recorder_camera2_id, rotation, ext, l_rec_env, res_front, res_rear, l_rec_env.bAudioRecEnable);
	/////////////////////////////////////////////////////////////////////////
	
	ui::PreviewRef preview1 = recorder_create_osd(parameters);

	//////////////////////////////////////////////////////////////////////////////////////////
#if 1
	std::shared_ptr<MyPreviewObserver> preview_observer = std::make_shared<MyPreviewObserver>();
	if (preview_observer->init() == 0) {
		ui::setPreviewObserver(preview1, preview_observer);
	}
#endif
#if 1
	...
	{
		ui::previewEnableMotionDetect(preview1, l_recorder_camera1_id, true);

		if(l_recorder_camera2_id >= 0)
			ui::previewEnableMotionDetect(preview1, l_recorder_camera2_id, true);
	}

	ui::previewSetMotionDetectSensitivityLevel(preview1, l_rec_env.motionSensi);

	ui::previewSetMotionDetectStateSize(preview1, 5);
	ui::previewSetMotionDetectThreshold(preview1, MD_THRESHOLD);
	{
		float window_scale, window_threshold;
		ui::previewGetMotionDetectWindowScale(preview1, l_recorder_camera1_id, window_scale);
		DLOG(DLOG_INFO, "before set, camera0 md window scale %.2f\n", window_scale);
		
		ui::previewGetMotionDetectWindowThreshold(preview1, l_recorder_camera1_id, window_threshold);
		DLOG(DLOG_INFO, "before set, camera0 md window threshold %.2f\n", window_threshold);

		if(l_recorder_camera2_id >= 0){
			ui::previewGetMotionDetectWindowScale(preview1, l_recorder_camera2_id, window_scale);
			DLOG(DLOG_INFO, "before set, camera1 md window scale %.2f\n", window_scale);
			ui::previewGetMotionDetectWindowThreshold(preview1, l_recorder_camera2_id, window_threshold);
			DLOG(DLOG_INFO, "before set, camera1 md window threshold %.2f\n", window_threshold);
		}
	}
	
	ui::previewSetMotionDetectWindowScale(preview1, 2.0);
	ui::previewSetMotionDetectWindowThreshold(preview1, 2.5);
	{
		float window_scale, window_threshold;
		ui::previewGetMotionDetectWindowScale(preview1, l_recorder_camera1_id, window_scale);
		DLOG(DLOG_INFO, "after set before start, camera0 md window scale %.2f\n", window_scale);
		ui::previewGetMotionDetectWindowThreshold(preview1, l_recorder_camera1_id, window_threshold);
		DLOG(DLOG_INFO, "after set before start, camera0 md window threshold %.2f\n", window_threshold);

		if(l_recorder_camera2_id >= 0){
			ui::previewGetMotionDetectWindowScale(preview1, l_recorder_camera2_id, window_scale);
			DLOG(DLOG_INFO, "after set before start, camera1 md window scale %.2f\n", window_scale);
			ui::previewGetMotionDetectWindowThreshold(preview1, l_recorder_camera2_id, window_threshold);
			DLOG(DLOG_INFO, "after set before start, camera1 md window threshold %.2f\n", window_threshold);
		}
	}
#endif

#if ENABLE_ADAS
	//set vanish line here before call previewStart()
	ui::previewSetAdasFrameMode(preview1, 0);
#endif
...
//	ui::previewShowCameraStream(preview1, 1, false);
	...
	////++}*************************************************************
	if(l_recorder_camera2_id >= 0)
		rear_camera_detected = recorder_camera_check(l_recorder_camera2_id, l_rec_env.rear_resolution);
	...
	if(rear_camera_detected){
		...
		recorder_parameters_set(parameters, l_recorder_camera1_id, l_recorder_camera2_id, rotation, ext, l_rec_env, res_front, res_rear, l_rec_env.bAudioRecEnable);
	}
	else {
		rear_disabled_ = true;
		recorder_parameters_set(parameters, l_recorder_camera1_id, -1, rotation, ext, l_rec_env, res_front, res_rear, l_rec_env.bAudioRecEnable);
	}

#if 1
	l_recorder_ref = createRecorder(VIDEO_NORMAL_DIR, parameters);
	...
	if(rotation == 0 && G_sensorGetControl()->rotate == DX_ROTATE_180)
	{
		err = startRecording(l_recorder_ref, recording_observer, nullptr, true, nullptr, 0);
		if(err < 0) {
	...
	sprintf(ma_strd_data_header_.ChannelInfo, "CH:%d", rear_camera_detected ? 2 : 1);
...
#ifdef DEF_MMB_SERVER
	{
		std::string _name("/acc_on");
		pai_r_datasaver_addLocation_new(_name, eMmbEventType_ACC_ON, 0);
	}
#endif
...
//		bool is_sd_ok = false;
			...
			switch(cmd.cmd_) {
...
#if 1	//1ch 동작 시 LED blink 구분 // test_231204
							led_set_normal_recording_start();
#else
							if(rear_camera_detected) //add
								led_set_normal_recording_start();
							else
								led_process_postCommand(LED_RED, 1000, 1000);
#endif
						}
						
						led_process_postCommand(LED_WORK_MODE, LED_WORK_NORMAL, LED_TIME_INFINITY);
						SYSTEMLOG(LOG_TYPE_SYSTEM, LOG_EVENT_REC_BEGIN ,0 , "M:%d CH:%d", l_rec_env.RecordingMode, rear_disabled_ == true ? 1 : 2);
						DLOG(DLOG_TRACE, "recording started\n");

						if(l_net_exist)
							pai_r_datasaver_send_msg_to_httpd(QMSG_CHANNEL, rear_disabled_ == true ? 1 : 2, 0, 0);
				...
				case kRestartRecording: {
					...
					if(l_is_recording) {						
						...
						if(is_record_restart_good++ < 10) { //evdev에서 500 delay 적용 됨, (5 + 10) * 100ms
							msleep(100);
							RecorderPostCommand(cmd.cmd_);
							break;
						}
				...
				case kStartEventGSensorLevelC:
				{
					...
					if(l_sys_setup.cfg.iEventMode == 0) { // 상시녹화 전용
					...
					u8 mmbEventType = 0; // 1:SOS, 2:급가속, 3:급감속, 4:급핸들, 5:충격, 6 over speed, 7 fast speed, 9:ACC ON, 10: ACC OFF
					...
					if(cmd.cmd_ == kStartEventRecording || (cmd.cmd_ >= kStartEventGSensorLevelA && cmd.cmd_ <= kStartEventGSensorLevelC)){
						mmbEventType = eMmbEventType_GSENSOR;
#ifdef DEF_MMB_VER_2
						l_datalog.m_event_cause_category = EVENT_CAUSE_CATEGORY_GSENSOR;
						l_datalog.m_event_cause_difference = 0;
#endif
					}
					if(cmd.cmd_ >= kStartEventSuddenAcceleration && cmd.cmd_ <= kStartEventGSensorLevelC){
						...
						switch (cmd.cmd_) {
							case 	kStartEventSuddenAcceleration:
							case 	kStartEventSuddenStart:	
								mmbEventType = eMmbEventType_RAPID_ACCELERATION;
								...
								break;
							case 	kStartEventSuddenDeacceleration:
								mmbEventType = eMmbEventType_SUDDEN_DECELERATION;
								l_datalog.m_event_cause_category = EVENT_CAUSE_CATEGORY_SUDDEN_DECELERATION;	
								l_datalog.m_event_cause_difference = safe_driving_get_diff_speed();
								break;
							case 	kStartEventRapidLeft:
							case 	kStartEventRapidRight:
								mmbEventType= eMmbEventType_SUDDEN_HANDLE;
								...
								break;
							case 	kStartEventOverSpeed:
								mmbEventType = eMmbEventType_OVERSPEED;

								l_datalog.m_event_cause_category = EVENT_CAUSE_CATEGORY_OVERSPEED;	
								l_datalog.m_event_cause_difference = safe_driving_get_overspeed_count();
								break;
							case 	kStartEventFastSpeed:
								mmbEventType = eMmbEventType_FASTSPEED;
								l_datalog.m_event_cause_category = EVENT_CAUSE_CATEGORY_FASTSPEED;	
								l_datalog.m_event_cause_difference = safe_driving_get_overspeed_count();
								break;
					...
					if(startEventRecording(l_recorder_ref, file_path.c_str(), &ma_strd_data_header_, sizeof(ma_strd_data_header_), auto_stop_and_restart_normal_recording_) < 0) {
						...
						datool_ext_trigger_out_set(1); // 1초만 High
						mixwav_play(kMixwaveEventRecording);
...
#ifdef SOS_CHECK
							if(cmd.cmd_ == kStartEventInput2)
								mmbEventType = eMmbEventType_SOS;	//Only SOS Event Upload
							else
								mmbEventType = 0;
#endif						
							pai_r_datasaver_addLocation_new(file_path, mmbEventType, PRE_RECORDING_SECONDS);
				...
				case kStartMotionRecording: {
					...
					file_path = Recorder_get_recording_file_name(VIDEO_MOTION_DIR, tm_t, l_normal_recording_type, rear_disabled_ ? 1 : 2, ext);
				...
				case kExternalPowerGood : {
					...
					if(power_disconnected){
#if 0 // 전원 Off/On시 멈추는 현상이 있어 사용하지 않음
#if DEF_SUB_MICOM_USE		
	...
	rec_text_thread = std::thread([&]() {
		...
		std::string title;
		u32 l_pulse_tmout_ms = 0;// user_data 1초에 1개 저장
		u32 uni_log_tmout_ms = 0;// UNI 농공대학
		u32 sos_event_time_sec = 0;
...
//		char stream_data[512] = { 0, };
#ifdef ENABLE_DATA_LOG_UNI
		time(&t);
		l_datalog.init(t);// Compensation in case of missing data for 20 seconds
#endif
		...
		do {
			...
			localtime_r(&t, &tm_t);
#ifdef ENABLE_DATA_LOG_UNI							
			if(is_time_out_ms(&uni_log_tmout_ms, 66)){ //test_1s=>15ea log save
				uni_log_tmout_ms -= uni_log_tmout_ms % 66; //timer 보정
				Recorder_datalog_add(t);
			}																				
#endif
			...
			//Recorder_datalog_add(t);
				...
				if(tm_t.tm_sec % 10 == 9){ // 10초에 한 번씩 저장
			...
			if(l_is_recording && recorderIsRunning(l_recorder_ref)) {
				...
				if(l_sys_setup.cfg.bOsdSpeed) {
					if(pulse.m_bPulseState && pulse.m_pulse_count_accrue)
				 		title.append(oasis::format("              %03dkm/h", (int)pulse.m_fPulseSpeed));
					else if(pulse.m_bGpsSignalState)
						title.append(oasis::format("              %03dkm/h", (int)pulse.m_fGpsSpeed));
				}
...
#ifdef DEF_OSAKAGAS_USERDATA_1SEC
				if(is_time_out_ms(&l_pulse_tmout_ms, 900)){//Osaks Gas 1초에 1번 저장
					evdev_input_pulse_data_add_stream();
					printf("evdev_input_pulse_data_add_stream excute 2 \n");
				}
#else
				evdev_input_pulse_data_add_stream();
#endif
...
#if !REAR_CAMERA_CHECK_ONFRAME			
				if(rear_camera_detected == false && l_recorder_camera2_id >= 0){
					rear_camera_detected = recorder_camera_check(l_recorder_camera2_id, l_rec_env.rear_resolution);
				}
#endif				
#ifdef DEF_REAR_CAMERA_ONLY
...
#ifdef SOS_CHECK	
					if((sos_event_time_sec++) == 5 && (int)pulse.m_bBgr == 1) {//Input2 'H'&5sec after booting
